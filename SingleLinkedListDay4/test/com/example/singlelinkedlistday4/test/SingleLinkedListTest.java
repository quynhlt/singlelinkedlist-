 ...
 * 
 */
package com.example.singlelinkedlistday4.test;
...
import com.example.singlelinkedlistday4.Node;
import com.example.singlelinkedlistday4.SingleLinkedList;
...
public class SingleLinkedListTest extends TestCase {
	...
	// 10
	public void testGetFindNodeFromList() {
		List<Node> nodes = createNewNodes();
		SingleLinkedList list = new SingleLinkedList(nodes);
		int position = 2;
		Node expectedNode = list.get(position);

		Node actualNode = list.find(expectedNode);
		assertEquals(expectedNode, actualNode);
	}
	...
	private List<Node> createNewNodes() {
		...
		for (int i = 0; i < n; i++) {
			...
			int nextIndex = i < n - 1 ? i + 1 : -1;
			int lastIndex = i > 0 ? i - 1 : -1;
